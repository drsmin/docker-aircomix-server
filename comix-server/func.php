...
################################################################################
$parent_path = "/data/manga";


$is_debug = false;
$source_encoding = "EUC-KR";
$target_encoding = "UTF-8";
$hidden_fullname = array(".", "..", "@eaDir", "Thumbs.db", ".DS_Store", '[Cover].jpg');
$hidden_partname = array("__MACOSX", '[Cover].jpg');
...
function list_dir($dir_path) {
	...
	for ($ix = 0 ; $ix < $cnt ; $ix++)
        {
              if(is_support($f[$ix], is_dir($dir_path.'/'.$f[$ix]) ) == true)
              {
                  echo $f[$ix]."\n";
                  //debug($f[$ix]);
              }
